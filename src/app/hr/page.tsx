      ...
      <section className="mb-6 grid gap-4 xl:grid-cols-[1fr_360px]">
        <div className="rounded-md border border-line bg-white p-5">
          <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex w-full min-w-0 items-center gap-2 rounded-md border border-line bg-paper px-3 py-2 text-sm text-steel sm:min-w-72">
              <Search className="h-4 w-4" />
              이름, 부서, 직무 검색
            </div>
            <div className="flex flex-wrap gap-2">
              {["전체", "재직", "휴직", "퇴사"].map((item) => (
                <button
                  key={item}
                  className={[
            ...
            <table className="min-w-[760px] w-full border-collapse text-left text-sm">
              ...
              <tbody className="divide-y divide-line bg-white">
                {employees.length ? employees.map((employee) => (
                  ...
                  <tr>
                    <td colSpan={5} className="px-4 py-10 text-center text-sm font-medium text-steel">등록된 직원이 없습니다.</td>
                  </tr>
                )}
              </tbody>
            </table>
