...
export function HrManagementPanel({ employees, departmentSummary }: { employees: EmployeeView[]; departmentSummary: Array<{ label: string; count: number }> }) {
  ...
  return <>
    <section className="mb-6 grid gap-4 xl:grid-cols-[1fr_360px]"><div className="min-w-0 rounded-md border border-line bg-white p-5"><ResponsiveFilterBar searchLabel="직원 검색" searchPlaceholder="이름, 부서, 직무 검색" searchValue={query} onSearchChange={setQuery} options={["전체", "재직", "휴직", "퇴사"]} selectedOption={selectedStatus} onOptionChange={setSelectedStatus} /><div className="hidden overflow-x-auto rounded-md border border-line md:block"><table className="min-w-[760px] w-full border-collapse text-left text-sm"><thead className="bg-paper text-steel"><tr><th className="px-4 py-3 font-medium">직원</th><th className="px-4 py-3 font-medium">부서/직무</th><th className="px-4 py-3 font-medium">입사일</th><th className="px-4 py-3 font-medium">연락처</th><th className="px-4 py-3 font-medium">상태</th><th className="px-4 py-3 font-medium">관리</th></tr></thead><tbody className="divide-y divide-line bg-white">{filteredEmployees.length ? filteredEmployees.map((employee) => <tr key={employee.id} className="hover:bg-paper"><td className="px-4 py-4"><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-md bg-paper text-marine"><UserRoundCog className="h-5 w-5" /></span><p className="font-bold text-ink">{employee.name}</p></div></td><td className="px-4 py-4"><p className="font-medium text-ink">{employee.department}</p><p className="mt-1 text-xs text-steel">{employee.role}</p></td><td className="px-4 py-4 text-steel">{employee.joined}</td><td className="px-4 py-4"><div className="flex flex-col gap-1 text-xs text-steel"><span className="inline-flex items-center gap-1"><Phone className="h-3 w-3" />{employee.phone}</span><span className="inline-flex items-center gap-1"><Mail className="h-3 w-3" />{employee.email}</span></div></td><td className="px-4 py-4"><StatusBadge status={employee.status} /></td><td className="px-4 py-4"><EmployeeRowActions employeeId={employee.id} employeeName={employee.name} archived={employee.status === "퇴사"} /></td></tr>) : <tr><td colSpan={6} className="px-4 py-10 text-center text-sm font-medium text-steel">{employees.length ? "조건에 맞는 직원이 없습니다." : "등록된 직원이 없습니다."}</td></tr>}</tbody></table></div><div className="space-y-3 md:hidden">{filteredEmployees.length ? filteredEmployees.map((employee) => <div key={employee.id} className="rounded-md border border-line bg-paper/60 p-4"><div className="flex items-start justify-between gap-3"><div className="flex min-w-0 items-center gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-md bg-white text-marine"><UserRoundCog className="h-5 w-5" /></span><div className="min-w-0"><p className="truncate font-bold text-ink">{employee.name}</p><p className="mt-1 truncate text-xs text-steel">{employee.department} · {employee.role}</p></div></div><div className="flex shrink-0 flex-col items-end gap-1"><StatusBadge status={employee.status} /><EmployeeRowActions employeeId={employee.id} employeeName={employee.name} archived={employee.status === "퇴사"} /></div></div><div className="mt-3 grid grid-cols-2 gap-2 text-xs"><div className="rounded-md bg-white px-3 py-2"><p className="text-steel">입사일</p><p className="mt-1 font-bold text-ink">{employee.joined}</p></div><div className="rounded-md bg-white px-3 py-2"><p className="text-steel">연락처</p><p className="mt-1 truncate font-bold text-ink">{employee.phone}</p></div></div><p className="mt-3 truncate text-xs text-steel">{employee.email}</p></div>) : <div className="rounded-md border border-line bg-paper px-4 py-10 text-center text-sm font-medium text-steel">{employees.length ? "조건에 맞는 직원이 없습니다." : "등록된 직원이 없습니다."}</div>}</div></div><aside className="space-y-4"><section className="rounded-md border border-line bg-white p-5"><h3 className="mb-4 font-bold text-ink">부서 구성</h3><div className="space-y-3">{departmentSummary.length ? departmentSummary.map((item) => <div key={item.label} className="flex items-center justify-between rounded-md bg-paper px-3 py-3"><p className="text-sm font-medium text-ink">{item.label}</p><p className="text-sm font-bold text-marine">{item.count}명</p></div>) : <p className="rounded-md bg-paper px-3 py-4 text-sm text-steel">등록된 부서가 없습니다.</p>}</div></section></aside></section>
  </>;
}
